...
@section('content')
    <div class="content">
        ...
        <form action="{{ route('roles.store') }}" method="POST" id="roles_form">
            @csrf
                    ...
                    <div class="row">
                        ...
                        <div class="col-lg-6 space-y-2">
                            ...
                            <select name="group_id" id="group_id"
                                class="js-example-basic-single form-control select select2" style="height: 400px;">
                        ...
                        $.ajax({
                            ...
                            success: function(response) {
                                ...
                                $.each(response, function(index, permission) {
                                    permissionsHtml += '<tr>';
                                    permissionsHtml += '<td class="parent-only"><h4>' +
                                        permission.name + '</h4></td>';
                                    permissionsHtml +=
                                        '<td><label  class="form-check-label">' + permission
                                        .name + '</label></td>';
                                    permissionsHtml +=
                                        '<td><input type="checkbox" name="permission[]" id="' +
                                        permission.id + '" value="' + permission.id +
                                        '" data-check_value="' + permission.id +
                                        '" data-parent_id="false" data-has_child="true" class="form-check-input parent myCheckbox"></td>';

                                    $.each(permission.subMenus, function(index1,
                                        permission1) {
                                        permissionsHtml += '<tr>';
                                        ...
                                        permissionsHtml +=
                                            '<td><input type="checkbox" name="permission[]" id="' +
                                            permission1.id + '" value="' +
                                            permission1.id +
                                            '" data-check_value="' + permission.id +
                                            '" data-parent_id="' + permission1
                                            .parent_id +
                                            '" data-has_child="true" class="form-check-input child myCheckbox"></td>';
                                        permissionsHtml += '</tr>';

                                        $.each(permission1.subMenus, function(
                                            index2, permission2) {
                                            permissionsHtml += '<tr>';
                                            permissionsHtml +=
                                            ...
                                            permissionsHtml +=
                                                ...
                                                permission.id +
                                                '" data-parent_id="' +
                                                permission2.parent_id +
                                                '" data-has_child="false" class="form-check-input subchild myCheckbox"></td>';
                                            permissionsHtml += '</tr>';
